...
def build_cubic_sampling_stencils(
    ...
    grid_spacing: Union[float, Iterable[float]],
    *,
    cell_spacing: Union[float, Iterable[float], None] = None,
    rotation_axis: torch.Tensor | None = None,
    rotation_angle_degrees: float = 0.0,
    dtype: torch.dtype = torch.float32,
    device: torch.device | str | None = None,
    ...
    ----------
    kernel_size
        Lagrange interpolation stencil size.
    grid_size
        Number of output sampling points on each axis.
    grid_spacing
        Spacing between output sampling points.
    cell_spacing
        Spacing of the source lattice that the interpolation stencil samples from.
        If omitted, ``grid_spacing`` is reused.
    """
    sampling_spacing = _to_3tuple(grid_spacing)
    source_spacing = _to_3tuple(grid_spacing if cell_spacing is None else cell_spacing)
    cell = torch.diag(torch.tensor(source_spacing, dtype=dtype, device=device))
    points = build_cubic_sampling_points(
        grid_size=grid_size,
        grid_spacing=sampling_spacing,
